...
dfkdd_train = pd.read_table("https://raw.githubusercontent.com/dimtics/Network-Intrusion-Detection-Using-Machine-Learning-Techniques/master/NSL_KDD_dataset/KDDTrain.txt", sep=",", names=datacols) 
# removes an unwanted extra field 
dfkdd_train = dfkdd_train.iloc[:,:-1] 

# Load NSL_KDD test dataset 
dfkdd_test = pd.read_table("https://raw.githubusercontent.com/dimtics/Network-Intrusion-Detection-Using-Machine-Learning-Techniques/master/NSL_KDD_dataset/KDDTest.txt", sep=",", names=datacols) 
...
mapping = {'ipsweep': 'Probe','satan': 'Probe','nmap': 'Probe','portsweep': 'Probe','saint': 'Probe','mscan': 'Probe', 'teardrop': 'DoS','pod': 'DoS','land': 'DoS','back': 'DoS','neptune': 'DoS','smurf': 'DoS','mailbomb': 'DoS', 'udpstorm': 'DoS','apache2': 'DoS','processtable': 'DoS', 'perl': 'U2R','loadmodule': 'U2R','rootkit': 'U2R','buffer_overflow': 'U2R','xterm': 'U2R','ps': 'U2R', 'sqlattack': 'U2R','httptunnel': 'U2R', 'ftp_write': 'R2L','phf': 'R2L','guess_passwd': 'R2L','warezmaster': 'R2L','warezclient': 'R2L','imap': 'R2L', 'spy': 'R2L','multihop': 'R2L','named': 'R2L','snmpguess': 'R2L','worm': 'R2L','snmpgetattack': 'R2L', 'xsnoop': 'R2L','xlock': 'R2L','sendmail': 'R2L', 'normal': 'Normal' } 
...
sc_traindf = dfkdd_train.select_dtypes(include=['float64','int64'])  #numerical data 
refclasscol = pd.concat([sc_traindf, enctrain], axis=1).columns #encoded string +numerical data-- col names 
refclass = np.concatenate((sc_train, enctrain.values), axis=1) #up but actual data 
X = refclass 
...
X_res, y_res = ros.fit_sample(X, y)  #whole dataset, attack class (both as 1d array)
...
# Transform train 
data X_train_1hotenc = enc.transform(Xresdfcat).toarray() 

#fit test data
 enc.fit(Xtestcat) 

 # Transform test
 data X_test_1hotenc = enc.transform(Xtestcat).toarray()
...
from sklearn.linear_model import LogisticRegression
 ...
 Model KNN_Classifier = KNeighborsClassifier(n_jobs=-1) 
...
combined_model = [('Naive Baye Classifier', BNB_Classifier), ('Decision Tree Classifier', DTC_Classifier), ('KNeighborsClassifier', KNN_Classifier), ('LogisticRegression', LGR_Classifier) ] 
...
print() 
print('============================== {} {} Model Evaluation =============================='.format(grpclass, i)) 
print() print ("Cross Validation Mean Score:" "\n", scores.mean()) 
#mean of each fold on cross validating 
print() 
print ("Model Accuracy:" "\n", accuracy) 
print() 
